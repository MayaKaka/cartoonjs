var apitest = {
	...
	webgl: {
		init: function(ct, dom, cvs, $fps) {
			...
			html.push('<style>#control select { margin: 10px auto; } #control select, #control input { color: #000; } #control label { padding: 0 5px; } #control input[type="number"] { width: 50px; }</style>');
			...
			var cube = world3d.add('cube');
			cube.position.set(0, 50, 0);
			
			var sphere = world3d.add('sphere', { radius: 20 });
			sphere.position.set(100, 100, 150);
			
			var plane = world3d.add('plane');
			plane.position.set(0, 0, 0);
			plane.rotation.set(-90*RAD_P_DEG, 0, 0);
			
			var light = world3d.add('light');
			light.position.set(0, 250, 0);
			
			var ring = world3d.add('ring');
			ring.position.set(100, 100, 150);
	...
	model: {
		init: function(ct, dom, cvs, $fps) {
			...
			var light = world3d.add('light', { strong: 2.5 });
			light.position.set(0, 200, 0);
			
			var plane = world3d.add('plane', { width: 4000, height: 2000, texture: 'images/grasslight_big.jpg' });
			plane.position.set(0, 0, 0);
			plane.rotation.set(-90*RAD_P_DEG, 0, 0);
			...
			world3d.add('model', { url: 'json/knight.js', type: 'skinned' });
			var bear;
			world3d.add('model', { url: 'json/bear.js', type: 'morphAnim',
				onload: function(obj) {
					bear = obj;
			...
			world3d.add('model', { url: 'json/ground.js', 
				onload: function(obj) {
					obj.position.set(0,20,-320);
					obj.scale.set(20,20,20);
			 	}
			});
			world3d.add('model', { url: 'json/gastonLagaffe.js',
				onload: function(obj) {
